...
#SBATCH --output=/scratch/project_00000000/test-time-facts-cache/push_to_hub_%A_%a.out
#SBATCH --error=/scratch/project_00000000/test-time-facts-cache/push_to_hub_%A_%a.ERROR.out
...
for i in "${!DIRS[@]}"; do
    ...
    srun --ntasks=1 --exclusive \
         singularity exec -B /scratch/project_00000000/test-time-facts-cache \
         "$CONTAINER" \
