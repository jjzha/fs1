...
#SBATCH --output=/scratch/project_000000000/cache/logs/inference_vllm_parallel_%A_%a.out
#SBATCH --error=/scratch/project_000000000/cache/logs/inference_vllm_parallel_%A_%a.err
...
OUTPUT_DIR="$SCRATCH/data/outputs-parallel"
PROMPT_FILE="$DATA_DIR/prompts-fs1-2708-parallel.jsonl"
LOG_DIR="$SCRATCH/logs"
mkdir -p $DATA_DIR $OUTPUT_DIR $LOG_DIR

# --- Model Definitions ---
# 💡 FIX: Removed the redundant second definition.
MODELS=(
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    ...
    srun singularity exec -B /scratch/project_000000000/ \
        $CONTAINER \
        bash -c "\$WITH_CONDA; \
        python3 prepare_vllm_batch_parallel.py \
...
TP_SIZE=4; PP_SIZE=1; TEMPERATURE=0.6; TOP_P=0.95; TOP_K=40;
...
srun singularity exec -B /scratch/project_000000000/ \
    ...
    python3 run_vllm_inference.py \
        ...
        --output_dir '$OUTPUT_DIR/${safe_model}-16' \
